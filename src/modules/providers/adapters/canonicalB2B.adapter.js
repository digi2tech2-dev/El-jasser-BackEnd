...
// Adapter for another deployment's complete canonical base URL, e.g.
// https://provider.example/client/api. It never appends a path to baseUrl.
...
class CanonicalB2BAdapter extends BaseProviderAdapter {
    ...
    async getProducts() {
        ...
        if (!Array.isArray(products)) throw new Error('[CanonicalB2B] GET /products returned an invalid product list');

        return products.map((product) => {
            const currency = product.currency == null ? null : String(product.currency).trim().toUpperCase();
            if (currency && currency !== 'USD') {
                throw new Error(`[CanonicalB2B] Product ${product.id ?? '<unknown>'} declares ${currency}; the provider price pipeline supports USD only.`);
            }
    ...
    async checkOrderByReference(referenceId) {
        ...
        const item = this._readCheckItems(data).find((candidate) => String(candidate?.order_uuid || '') === String(referenceId));
    ...
    async _recoverUncertainPlacement(referenceId, placementError) {
        try {
            ...
            return { success: true, providerOrderId: null, providerStatus: 'PLACEMENT_UNCERTAIN', rawResponse: { placement: 'uncertain', recovery: 'not_found', httpStatus: getErrorDetails(placementError).httpStatus }, errorMessage: null };
        } catch (recoveryError) {
            return { success: true, providerOrderId: null, providerStatus: 'PLACEMENT_UNCERTAIN', rawResponse: { placement: 'uncertain', recovery: 'lookup_failed', httpStatus: getErrorDetails(placementError).httpStatus, recoveryHttpStatus: getErrorDetails(recoveryError).httpStatus }, errorMessage: null };
        }
    }
    ...
    async placeOrder(params = {}) {
        ...
        if (!/^\d+$/.test(externalProductId) || Number(externalProductId) <= 0) return makeFailureResult({ message: 'Canonical B2B product ID must be a positive numeric compatibility ID', rawResponse: { validation: 'invalid_product_id' } });
        if (!referenceId) return makeFailureResult({ message: 'Canonical B2B referenceId is required', rawResponse: { validation: 'missing_reference_id' } });

        const { externalProductId: _externalProductId, providerProductId: _providerProductId, quantity, amount: _amount, productId: _productId, referenceId: _referenceId, price: _price, basePrice: _basePrice, providerPrice: _providerPrice, walletBalance: _walletBalance, balance: _balance, currency: _currency, ...mappedFields } = params;
        const payload = { product_id: Number(externalProductId), qty: quantity, order_uuid: referenceId, params: mappedFields };
        try {
            const { data } = await this._client.post('/orders', payload);
            const order = data?.data;
            if (data?.status !== 'OK' || !order?.order_id) return makeFailureResult({ message: order?.message ?? data?.message ?? 'Canonical B2B provider rejected the order', rawResponse: data });
            ...
            if (isUncertainPlacementError(error)) return this._recoverUncertainPlacement(referenceId, error);
            const details = getErrorDetails(error);
            return makeFailureResult({ message: details.body?.message ?? details.message, rawResponse: { httpStatus: details.httpStatus, code: details.body?.code ?? null, message: details.body?.message ?? details.message } });
        }
    }
    ...
    async checkOrders(orderIds) {
        ...
        return this._readCheckItems(data).map((item) => this._normaliseCheckItem(item)).filter((item) => item && requested.has(String(item.providerOrderId)));
    }
