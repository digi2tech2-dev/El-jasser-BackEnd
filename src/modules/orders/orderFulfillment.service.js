...
const executeOrder = async (orderId, provider = null, auditContext = null) => {
    ...
    try {
        result = await resolvedProvider.placeOrder({
            ...
            // Stable provider idempotency reference. Never regenerate it on
            // placement retries or recovery checks.
        ...
        if (isTransient) {
            ...
            result = {
                ...
                rawResponse: { placement: 'uncertain', message: err.message, isTransient: true },
                errorMessage: null,
            };
        } else {
...
// A response lost after dispatch is not a provider rejection. Keep the order
// funded and processing until reference lookup either recovers it or retries
// are exhausted into manual review.
const recoverUncertainPlacement = async (order, adapter) => {
    ...
    if (typeof adapter.checkOrderByReference !== 'function') return defer('unsupported');

    try {
        const result = await adapter.checkOrderByReference(order.orderNumber);
        if (!result?.found || !result.providerOrderId) return defer('not_found');
...
const pollProcessingOrders = async (providerOverride = null) => {
    ...
    // ── 1. Fetch normal provider IDs plus uncertain placement attempts ───────
